...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchProductData = async () => {
      ...
      try {
        ...
        const relatedRes = await getProducts({ category: productData.category, limit: 4 });
  ...
  const addToCart = () => {
    ...
    const existingItemIndex = cart.findIndex((item: any) => item.product._id === product._id && item.size === selectedSize);
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white px-6 text-center">
        <h2 className="font-heading font-medium text-4xl  text-primary-deep mb-4">Không tìm thấy sản phẩm</h2>
        <p className="text-primary-deep/60 mb-8">The product you're looking for doesn't exist or has been removed.</p>
        <Link to="/products" className="px-8 py-3 bg-primary text-white text-xs uppercase tracking-widest font-bold rounded-full">
          Back to Shop
        </Link>
      </div>
    );
  ...
  return (
    <main className="bg-white font-body selection:bg-primary selection:text-white">
      ...
      <section className="bg-[#F5F5F5] py-4">
        <div className="max-w-[1440px] mx-auto px-6 md:px-12">
          <nav className="flex items-center gap-2 text-xs text-primary-deep/60">
            ...
            <Link to="/products" className="hover:text-primary transition-colors uppercase tracking-widest">Products</Link>
            <svg className="w-2 h-2" fill="currentColor" viewBox="0 0 320 512">
              <path d="M278.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L210.7 256 73.4 118.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l160 160z" />
            </svg>
            <span className="text-primary-deep font-medium uppercase tracking-widest">{product.name}</span>
          </nav>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-12 gap-12 lg:gap-20 items-start">
            ...
            <div className="md:col-span-7 sticky top-24">
              <div className="space-y-4">
                <motion.div 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="aspect-square bg-white overflow-hidden shadow-sm rounded-2xl"
                >
                  <img src={mainImage} alt={product.name} className="w-full h-full object-cover transition-transform duration-700 hover:scale-110" />
                </motion.div>
            ...
            <div className="md:col-span-5 sticky top-24">
              <div className="flex flex-col">
                <p className="text-xs uppercase tracking-[0.3em] text-primary/60 mb-2 font-bold">{product.brand}</p>
                <h1 className="font-heading font-medium text-4xl lg:text-5xl text-primary-deep  mb-4">{product.name}</h1>
                
                {/* Review Badge */}
                <div className="flex items-center gap-2 mb-6">
                  <div className="flex text-primary">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className={`w-3.5 h-3.5 fill-current ${i < Math.floor(product.rating) ? "text-primary" : "text-gray-300"}`} viewBox="0 0 20 20">
                        <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z" />
                      </svg>
                    ))}
                  </div>
                  <span className="text-[11px] text-primary-deep/40 uppercase tracking-widest">{product.rating > 0 ? `${product.rating} rating` : "No reviews"}</span>
                </div>

                {/* Price */}
                <div className="flex items-center gap-4 mb-8">
                  <span className="text-3xl font-body text-primary font-bold">{formatPrice(product.salePrice || product.price)}</span>
                  {product.salePrice && product.salePrice < product.price && (
                    <>
                      <span className="text-xl text-primary-deep/30 line-through">{formatPrice(product.price)}</span>
                      <span className="bg-primary text-white text-[10px] px-2 py-1 rounded-sm font-bold">
                ...
                <p className="text-primary-deep/70 leading-relaxed mb-8 text-sm">
                  {product.description}
                </p>
                ...
                <div className="space-y-8 mb-10">
                  {/* ML Options */}
                  <div>
                    <h3 className="text-xs uppercase tracking-widest font-bold text-primary-deep mb-4">Dung tích: <span className="text-primary">{selectedSize}</span></h3>
                        ...
                        <button
                          ...
                          className={`px-6 py-2.5 text-xs font-bold transition-all border ${
                            selectedSize === size 
                              ...
                              : "bg-white text-primary-deep/60 border-primary-deep/10 hover:border-primary/50"
                          }`}
                        >
                          {size}
                        </button>
                  ...
                  {product.scentNotes && product.scentNotes.length > 0 && (
                    <div>
                      <h3 className="text-xs uppercase tracking-widest font-bold text-primary-deep mb-4">Nhóm hương:</h3>
                      <div className="flex flex-wrap gap-2">
                        {product.scentNotes.map((note, idx) => (
                          <span key={idx} className="px-4 py-1.5 bg-white border border-black/5 rounded-full text-[10px] uppercase tracking-wider text-primary-deep/60">
                            {note}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    <h3 className="text-xs uppercase tracking-widest font-bold text-primary-deep">Trạng thái:</h3>
                ...
                <div className="space-y-6">
                  <div className="flex gap-4">
                    <div className="flex items-center border border-primary-deep/10 bg-white h-12 px-4 rounded-lg">
                      <button 
                    ...
                    <button 
                      onClick={addToCart}
                      className="flex-1 bg-white text-primary-deep border border-primary-deep font-bold text-xs uppercase tracking-widest h-12 hover:bg-primary hover:text-white hover:border-primary transition-all duration-500 rounded-lg"
                    >
                      Thêm vào giỏ
                    </button>
                  </div>
                  <button className="w-full bg-primary text-white font-bold text-xs uppercase tracking-[0.2em] h-12 hover:bg-primary-dark transition-all duration-500 shadow-lg rounded-lg">
                    Mua ngay
                  </button>
                </div>
            ...
            >
              {activeTab === "Description" && (
                <div className="space-y-20">
                  <div className="max-w-4xl mx-auto text-center">
                    <p className="text-lg  font-heading font-medium text-primary-deep leading-relaxed mb-12">
                  ...
                  <div className="grid md:grid-cols-2 gap-20 items-center">
                    <div className="space-y-8">
                      <h3 className="font-heading font-medium text-4xl  text-primary-deep">Đặc điểm nổi bật</h3>
                  ...
                  <div className="pt-20">
                    <h3 className="font-heading font-medium text-4xl  text-primary-deep text-center mb-12">Chất lượng sản phẩm đỉnh cao</h3>
              ...
              {activeTab === "Customer reviews" && (
                <div className="max-w-4xl mx-auto text-center py-12">
                  ...
                  <h3 className="text-2xl font-heading font-medium  text-primary-deep mb-2">Đánh giá từ khách hàng</h3>
      ...
      <section className="py-24 bg-[#FCFCFC]">
        <div className="max-w-[1440px] mx-auto px-6 md:px-12">
          <h2 className="font-heading font-medium text-4xl  text-primary-deep mb-16 text-center">Có Thể Bạn Cũng Thích</h2>
            ...
            {relatedProducts.map((product, idx) => (
              ...
              >
                <div className="aspect-[3/4] rounded-2xl overflow-hidden bg-white mb-6 relative shadow-sm border border-gray-100">
                  <img src={product.images[0] || "https://via.placeholder.com/600"} alt={product.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
          ...
          {relatedProducts.length === 0 && (
            <p className="text-center text-primary-deep/40 ">Không có sản phẩm liên quan nào.</p>
          )}
        </div>
      </section>
    </main>
